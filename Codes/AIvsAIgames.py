...
def ai_vs_ai(
    depth_ai1, depth_ai2, transposition_table_ai1, transposition_table_ai2, n_game
):
    ...
    print(
        f"Game number {n_game}: White: AI (Depth {depth_white}), Black: AI (Depth {depth_black})"
    )
    ...
    save_game(
        sequence,
        f"AI (Depth {depth_white})",
        f"AI (Depth {depth_black})",
        n_game,
        board,
    )
...
def n_games(depth1, depth2, number_games=10):
    ...
    for i in range(1, number_games + 1):
        result, depth_white, depth_black = ai_vs_ai(
            depth1, depth2, transposition_table_ai1, transposition_table_ai2, i
        )

        if (result == "1-0" and depth1 == depth_white) or (
            result == "0-1" and depth1 == depth_black
        ):
            wins += 1
        elif (result == "1-0" and depth2 == depth_white) or (
            result == "0-1" and depth2 == depth_black
        ):
            losses += 1
        else:
            draws += 1
