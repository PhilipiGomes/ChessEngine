...
class ChessEngine:
    def __init__(self, board: chess.Board, depth: int, color: bool):
        self.board = board
        self.depth = depth
    ...
    def is_endgame(self) -> bool:
        ...
        major_pieces = sum(1 for p in piece_map.values() if p.piece_type == chess.ROOK)
        return (
            queens == 0 or major_pieces <= 1 or (major_pieces == 2 and minor_pieces < 3)
        )

    def piece_value_type(self, piece):
    ...
    def Mop_up_eval(self) -> float:
        ...
        if enemy_square is None or friend_square is None:
            return 0.0
