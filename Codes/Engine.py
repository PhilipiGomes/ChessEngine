    ...
    if sequence:
        ...
        for opening, moves in op.items():
            if moves[: len(sequence)] == sequence and (
                len(moves) > len(sequence) if len(sequence) > 0 else 1 == 1
            ):
...
def move_priority(board: chess.Board, move: chess.Move) -> float:
    ...
    if board.is_attacked_by(not board.turn, move.to_square):
        guess -= (
            piece_value_type(attacker)
            if board.turn == chess.WHITE
            else -piece_value_type(attacker)
        )
...
def get_best_move(
    board: chess.Board, depth: int, sequence: List[str]
) -> Optional[chess.Move]:
    ...
    for move in moves:
        ...
        score = minimax_alpha_beta(
            depth - 1, -999999, 999999, board.turn == chess.WHITE, board, depth
        )
        board.pop()
