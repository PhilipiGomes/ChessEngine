...
def ai_vs_ai(
    ...
    if result == "1-0":  # White win
        ...
        ratings[f"AI (Depth {depth_white})"] = update_rating(
            ratings[f"AI (Depth {depth_white})"],
            ratings[f"AI (Depth {depth_black})"],
            1,
            0,
        )[0]
        ratings[f"AI (Depth {depth_black})"] = update_rating(
            ratings[f"AI (Depth {depth_white})"],
            ratings[f"AI (Depth {depth_black})"],
            1,
            0,
    ...
    for depth2 in depths[depths.index(depth1) + 1 :]:
        for _ in range(n_rounds):
            ai_vs_ai(
                ...
                results,
                ratings,
                transposition_tables[depth1],
                transposition_tables[depth2],
                game_number,
            )
...
for depth in sorted(ratings.keys(), key=lambda x: ratings[x], reverse=True):
    print(
        f"{depth}| {results[depth]['wins']}       | {results[depth]['draws']}     | {results[depth]['losses']}     | {ratings[depth]:.2f}"
    )
