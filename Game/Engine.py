...
class ChessEngine:
    ...
    def mopup_eval(self, my_king_square, opp_king_square):
        eval = 0
    ...
    def move_score(self, move: chess.Move):
        score = 0
    ...
    def move_ordering(self, moves: List[chess.Move]):
        return moves.sort(key=self.move_score, reverse=True)
    ...
    def get_best_move(self, sequence: List[str]) -> chess.Move:
        ...
        for move in moves:
            self.board.push(move)
            score = self.minimax(self.depth - 1)
            self.board.pop()
            if self.color == chess.WHITE:
                if score >= best_score:
                    best_score = score
